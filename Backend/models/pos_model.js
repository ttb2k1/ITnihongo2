...
pos_model.reserved = async (data) => {
    try
    {
      ...
      if(results.length>0)
      {
        var sql1 = "UPDATE positions SET user_id = ?, vehicle_id = ?, QR_code = ?, status=1, position_active=1 WHERE pos_id = ?"
  ...
  try
    {
      ...
      if(results.affectedRows>0)
      {
        var sql = "UPDATE histories SET check_in = Now() WHERE check_in = NULL and user_id = ?"
  ...
  try
    {
      ...
      if(results.affectedRows>0)
      {
        var sql = "UPDATE histories SET check_out = Now() WHERE user_id = ? and check_out=NULL"
        result = await query(sql,data_list)
        if(result.affectedRows>0)
        {
